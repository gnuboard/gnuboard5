...
<section id ="anc_scf_payment">
    ...
    <div class="tbl_frm01 tbl_wrap">
        ...
        <tr class="pg_info_fld nicepay_info_fld" id="nicepay_info_anchor">
            ...
            <td>
                <?php echo help("나이스페이로 부터 발급 받으신 상점아이디(MID) 10자리 중 SIR 을 제외한 나머지 7자리를 입력 합니다.\n만약, 상점아이디가 SIR로 시작하지 않는다면 계약담당자에게 변경 요청을 해주시기 바랍니다. (Tel. 02-3430-5858) 예) SIRpaytest"); ?>
        ...
        <tr class="pg_info_fld nicepay_info_fld">
            <th scope="row"><label for="de_nicepay_admin_key">나이스페이 취소패스워드</label></th>
            <td>
                <?php echo help("나이스페이에서 발급받은 10자리 취소패스워드를 입력합니다.\n나이스페이 가맹점관리자 패스워드와 관련이 없습니다.\n취소패스워드 값은 나이스페이 가맹점관리자 > 가맹점정보 > 비밀번호관리 > 거래취소비밀번호에서 변경 가능합니다."); ?>
        ...
        <tr class="pg_info_fld nicepay_info_fld">
            <th scope="row"><label for="de_nicepay_sign_key">나이스페이 암호화 key</label></th>
            <td>
                <?php echo help("나이스페이에서 발급받은 암호화 key를 입력합니다.\n나이스페이 가맹점관리자 > 가맹점정보 > KEY관리 > 하단의 Key값을 입력합니다."); ?>
...
if($default['de_iche_use'] || $default['de_vbank_use'] || $default['de_hp_use'] || $default['de_card_use']) {
    ...
    if($default['de_pg_service'] == 'nicepay') {
        ...
        try {
            if( ! is_dir($log_path) && is_writable(G5_SHOP_PATH.'/nicepay/') ){
                @mkdir($log_path, G5_DIR_PERMISSION);
                @chmod($log_path, G5_DIR_PERMISSION);
            }
        } catch(Exception $e) {
