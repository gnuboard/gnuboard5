...
$message = array("필수! 3번째 행부터 입력해주세요.<br>번호 : 빈칸으로 두세요.(권장)<br>주소 : 우편번호, 기본주소, 상세주소 3개 항목을 모두 입력하셔야 등록됩니다.<br>지번 : 등록한 주소지가 지번 주소인 경우 : Y 입력 / 등록하는 주소가 도로명인 경우 비워두거나<br>빨간색으로 표시된 항목은 필수입력 항목입니다.<br>예제 (아이디=test 인 행) 지우고 사용해 주세요.");
...
foreach($widths as $key => $var) {
    $excel->setActiveSheetIndex(0)->getColumnDimension(column_char($key))->setWidth($var);
}
