...
$sort1 = (isset($_GET['sort1']) && in_array($_GET['sort1'], ['pay_id', 'py_cart_price', 'py_receipt_price', 'py_cancel_price', 'py_misu', 'py_cash'])) ? $_GET['sort1'] : '';
$sort2 = (isset($_GET['sort2']) && in_array($_GET['sort2'], ['desc', 'asc'])) ? $_GET['sort2'] : 'desc';
$sel_field = (isset($_GET['sel_field']) && in_array($_GET['sel_field'], ['pay_id', 'mb_id', 'py_name', 'py_tel', 'py_hp', 'py_b_name', 'py_b_tel', 'py_b_hp', 'py_deposit_name', 'py_invoice'])) ? $_GET['sel_field'] : '';
...
if ($py_status) {
    switch ($py_status) {
        case '전체취소':
            ...
            break;
        case '부분취소':
            $where[] = " py_status IN('주문', '입금', '준비', '배송', '완료') and py_cancel_price > 0 ";
            break;
        default:
...
if ($py_settle_case) {
    if ($py_settle_case === '간편결제') {
        $where[] = " py_settle_case in ('간편결제', '삼성페이', 'lpay', 'inicis_kakaopay') ";
    } else {
        $where[] = " py_settle_case = '$py_settle_case' ";
    }
}
...
//$sql = " select *,
//            (py_cart_coupon + py_coupon + py_send_coupon) as couponprice
//           $sql_common
...
?>
<div class="admin_pg_notice py_test_caution">
...
<select name="sel_field" id="sel_field">
    <option value="pay_id" <?php echo get_selected($sel_field, 'pay_id'); ?>>주문번호</option>
...
<div>
    <strong>결제수단</strong>
    <input type="radio" name="py_settle_case" value="" id="py_settle_case01"        <?php echo get_checked($py_settle_case, ''); ?>>
    <label for="py_settle_case01">전체</label>
    <input type="radio" name="py_settle_case" value="무통장" id="py_settle_case02"   <?php echo get_checked($py_settle_case, '무통장'); ?>>
    <label for="py_settle_case02">무통장</label>
    <input type="radio" name="py_settle_case" value="휴대폰" id="py_settle_case05"   <?php echo get_checked($py_settle_case, '휴대폰'); ?>>
    <label for="py_settle_case05">휴대폰</label>
    <input type="radio" name="py_settle_case" value="신용카드" id="py_settle_case06" <?php echo get_checked($py_settle_case, '신용카드'); ?>>
    <label for="py_settle_case06">신용카드</label>
</div>
...
<div>
    ...
    <input type="checkbox" name="py_receipt_point" value="Y" id="py_misu04" <?php echo get_checked($py_receipt_point, 'Y'); ?>>
    <label for="py_misu04">포인트주문</label>
    <input type="checkbox" name="py_coupon" value="Y" id="py_misu05" <?php echo get_checked($py_coupon, 'Y'); ?>>
    <label for="py_misu05">쿠폰</label>
    <?php if ($default['de_escrow_use']) { ?>
    <input type="checkbox" name="py_escrow" value="Y" id="py_misu06" <?php echo get_checked($py_escrow, 'Y'); ?>>
    <label for="py_misu06">에스크로</label>
    <?php } ?>
</div>
...
<div class="tbl_head01 tbl_wrap">
    ...
    for ($i = 0; $row = sql_fetch_array($result); ++$i) {
        ...
        // 주문번호에 - 추가
        switch (strlen($row['pay_id'])) {
            case 16:
                $disp_pay_id = substr($row['pay_id'], 0, 8).'-'.substr($row['pay_id'], 8);
                break;
            default:
                $disp_pay_id = substr($row['pay_id'], 0, 6).'-'.substr($row['pay_id'], 6);
                break;
        }
    ...
    <tr class="orderlist<?php echo ' '.$bg; ?>">
        ...
        <td headers="th_ordnum" class="td_odrnum2" rowspan="2">
            <a href="<?php echo G5_SHOP_URL; ?>/orderinquiryview.php?pay_id=<?php echo $row['pay_id']; ?>&amp;uid=<?php echo $uid; ?>" class="orderitem"><?php echo $disp_pay_id; ?></a>
            <?php echo $py_paytype; ?>
        </td>
