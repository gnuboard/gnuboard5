...
if (in_array($_POST['pb_status'], $status_normal) || in_array($_POST['pb_status'], $status_cancel)) {
    ; // 통과
} else {
...
for ($i=0; $i<$cnt; $i++)
{
    ...
    /*
    $sql = " select * from {$g5['g5_subscription_pay_basket_table']} where od_id = '$od_id' and pb_id  = '$pb_id' ";
    $pb = sql_fetch($sql);
    */
    ...
    if ($pb['pb_stock_use'])
    {
        if ($pb_status == '주문' || $pb_status == '취소' || $pb_status == '반품' || $pb_status == '품절')
        {
            $stock_use = 0;
            // 재고에 다시 더한다.
            if($pb['io_id']) {
                $sql = " update {$g5['g5_shop_item_option_table']}
                            set io_stock_qty = io_stock_qty + '{$pb['pb_qty']}'
                            where it_id = '{$pb['it_id']}'
                              and io_id = '{$pb['io_id']}'
                              and io_type = '{$pb['io_type']}' ";
            } else {
                $sql = " update {$g5['g5_shop_item_table']}
                            set it_stock_qty = it_stock_qty + '{$pb['pb_qty']}'
                            where it_id = '{$pb['it_id']}' ";
            }
    ...
    else
    {
        ...
        else if ($pb_status == '품절') {
            $stock_use = 1;
            // 재고에서 뺀다.
            $sql =" update {$g5['g5_shop_item_table']} set it_stock_qty = 0 where it_id = '{$pb['it_id']}' ";
            sql_query($sql);
        } */
    }
    ...
    {
        $point_use = 0;
        //insert_point($mb_id, (-1) * ($pb[pb_point] * $pb[pb_qty]), "주문번호 $od_id ($pb_id) 취소");
    ...
    // it_id를 배열에 저장
    if($pb_status == '주문' || $pb_status == '취소' || $pb_status == '반품' || $pb_status == '품절' || $pb_status == '완료')
        $arr_it_id[] = $pb['it_id'];
}
