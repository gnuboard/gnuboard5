...
if ($posts['next_billing_date']) {
    if (isValidDate_ymd($posts['next_billing_date']) == false)
        alert('결제일이 날짜 형식이 아닙니다.');
}

if ($posts['next_delivery_date']) {
    if (isValidDate_ymd($posts['next_delivery_date']) == false)
        alert('배송일이 날짜 형식이 아닙니다.');
}
...
if (date('y-m-d', strtotime($posts['next_billing_date'])) !== date('y-m-d', strtotime($od['next_billing_date']))) {
    $add_where .= ", next_billing_date = '{$posts['next_billing_date']}' ";
        
    add_subscription_order_history($current_cycle_str. ' 결제일을 '. $posts['next_billing_date'] .' 으로 변경함', array(
        'hs_type' => 'change_subscription_order_billing',
        'od_id' => $od_id,
        'mb_id' => $member['mb_id']
    ));
}

if (date('y-m-d', strtotime($posts['next_delivery_date'])) !== date('y-m-d', strtotime($od['next_delivery_date']))) {
    $add_where .= ", next_delivery_date = '{$posts['next_delivery_date']}' ";
    
    add_subscription_order_history($current_cycle_str. ' 예정 배송일을 '. $posts['next_delivery_date'] .' 으로 변경함', array(
        'hs_type' => 'change_subscription_order_delivery',
        'od_id' => $od_id,
        'mb_id' => $member['mb_id']
    ));
}
