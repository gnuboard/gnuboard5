...
// 분류
$sc_list  = '<option value="">선택</option>'.PHP_EOL;
...
for ($i=0; $row=sql_fetch_array($result); $i++)
{
    ...
    $sc_list .= '<option value="'.$row['sc_id'].'">'.$nbsp.$row['sc_name'].'</option>'.PHP_EOL;
}
...
if ($sca != "") {
    $sql_search .= " $where (a.sc_id like '$sca%' or a.sc_id2 like '$sca%' or a.sc_id3 like '$sca%') ";
}

if ($sfl == "")  $sfl = "it_name";

$sql_common = " from {$g5['g5_subscription_item_table']} a ,
                     {$g5['g5_subscription_category_table']} b
               where (a.sc_id = b.sc_id";
if ($is_admin != 'super')
    $sql_common .= " and b.sc_mb_id = '{$member['mb_id']}'";
$sql_common .= ") ";
...
<select name="sca" id="sca">
    ...
    <?php
    $sql1 = " select sc_id, sc_name from {$g5['g5_subscription_category_table']} order by sc_order, sc_id ";
    $result1 = sql_query($sql1);
    for ($i=0; $row1=sql_fetch_array($result1); $i++) {
        $len = strlen($row1['sc_id']) / 2 - 1;
        $nbsp = '';
        for ($i=0; $i<$len; $i++) $nbsp .= '&nbsp;&nbsp;&nbsp;';
        echo '<option value="'.$row1['sc_id'].'" '.get_selected($sca, $row1['sc_id']).'>'.$nbsp.$row1['sc_name'].'</option>'.PHP_EOL;
    }
    ?>
</select>
...
<div class="tbl_head01 tbl_wrap">
    ...
    for ($i=0; $row=sql_fetch_array($result); $i++)
    {
        $href = subscription_item_url($row['it_id']);
        $bg = 'bg'.($i%2);
    ...
    <tr class="<?php echo $bg; ?>">
        ...
        <td colspan="5" class="td_sort">
            <label for="sc_id_<?php echo $i; ?>" class="sound_only"><?php echo get_text($row['it_name']); ?> 기본분류</label>
            <select name="sc_id[<?php echo $i; ?>]" id="sc_id_<?php echo $i; ?>">
                <?php echo conv_selected_option($sc_list, $row['sc_id']); ?>
            </select>
            <label for="sc_id2_<?php echo $i; ?>" class="sound_only"><?php echo get_text($row['it_name']); ?> 2차분류</label>
            <select name="sc_id2[<?php echo $i; ?>]" id="sc_id2_<?php echo $i; ?>">
                <?php echo conv_selected_option($sc_list, $row['sc_id2']); ?>
            </select>
            <label for="sc_id3_<?php echo $i; ?>" class="sound_only"><?php echo get_text($row['it_name']); ?> 3차분류</label>
            <select name="sc_id3[<?php echo $i; ?>]" id="sc_id3_<?php echo $i; ?>">
                <?php echo conv_selected_option($sc_list, $row['sc_id3']); ?>
            </select>
        </td>
        <td rowspan="3" class="td_num">
        ...
        <td rowspan="3" class="td_mng td_mng_s">
            <a href="./itemform.php?w=u&amp;it_id=<?php echo $row['it_id']; ?>&amp;sc_id=<?php echo $row['sc_id']; ?>&amp;<?php echo $qstr; ?>" class="btn btn_03"><span class="sound_only"><?php echo htmlspecialchars2(cut_str($row['it_name'],250, "")); ?> </span>수정</a>
            <a href="./itemcopy.php?it_id=<?php echo $row['it_id']; ?>&amp;sc_id=<?php echo $row['sc_id']; ?>" class="itemcopy btn btn_02" target="_blank"><span class="sound_only"><?php echo htmlspecialchars2(cut_str($row['it_name'],250, "")); ?> </span>복사</a>
