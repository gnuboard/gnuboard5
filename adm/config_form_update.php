...
// 이전 언어 설정 저장 (테이블 복사 시 비교용)
$previous_languages = !empty($ori_config['cf_lang_type']) ? explode(',', $ori_config['cf_lang_type']) : array();
...
$sql = " update {$g5['config_table']}
            set cf_title = '{$cf_title}',
                cf_admin = '{$cf_admin}',
                cf_robots = '{$_POST['cf_robots']}',
                cf_view_mode = '{$_POST['cf_view_mode']}',
                cf_dev_mode = '{$_POST['cf_dev_mode']}',
                cf_dev_ip = '{$_POST['cf_dev_ip']}',
                ...
                cf_admins_ip = '" . sql_real_escape_string(trim($_POST['cf_admins_ip'])) . "',
                cf_lang_type = '" . (isset($_POST['cf_lang_type']) && is_array($_POST['cf_lang_type']) && !empty($_POST['cf_lang_type']) ? sql_real_escape_string(implode(',', array_unique($_POST['cf_lang_type']))) : '') . "',
                ...
                cf_email_wr_site_admin = '{$_POST['cf_email_wr_site_admin']}',
                ...
                cf_email_mb_site_admin = '{$_POST['cf_email_mb_site_admin']}',
...
// 언어 설정이 변경된 경우 테이블 복사 처리
$selected_languages = isset($_POST['cf_lang_type']) && is_array($_POST['cf_lang_type']) ? $_POST['cf_lang_type'] : array();
$new_languages = array_diff($selected_languages, $previous_languages);

if (!empty($new_languages)) {
    // 새로 추가된 언어에 대해 테이블 복사
