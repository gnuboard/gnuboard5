...
$cart_action_url = G5_SUBSCRIPTION_URL . '/cartupdate.php';

// 테마에 cart.php 있으면 include
if (defined('G5_THEME_MSUBSCRIPTION_PATH')) {
    $theme_cart_file = G5_THEME_MSUBSCRIPTION_PATH . '/cart.php';
...
add_event('subscription_head_container_title', 'html_shop_head_container_title', 1, 1);

function html_shop_head_container_title($container_class) {
    global $subscription_cart_datas;
    ?>
    <ul class="cart-heading">
        <li class="linked-title is-shop-cart-link"><a href="<?php echo G5_SHOP_URL . '/cart.php'; ?>">쇼핑몰</a> <span class="cart-number"><?php echo get_boxcart_datas_count(); ?></span></li><li class="linked-title is-subscription-cart-link active">정기구독 <span class="cart-number"><?php echo $subscription_cart_datas ? count($subscription_cart_datas) : ''; ?></span></li>
    </ul>
    <?php
}
...
<div id="sod_bsk">
    ...
    <form name="frmcartlist" id="sod_bsk_list" class="2017_renewal_itemform" method="post" action="<?php echo $cart_action_url; ?>">
        ...
        <?php if ($subscription_cart_datas && count($subscription_cart_datas) > 0) { ?>
        ...
        <ul class="sod_list">
            ...
            foreach ($subscription_cart_datas as $row) {
                ...
                $a1 = '<a href="' . subscription_item_url($row['it_id']) . '"><strong>';
                $a2 = '</strong></a>';
                $image_width = 65;
                $image_height = 65;
                $image = get_subscription_it_image($row['it_id'], $image_width, $image_height);
            ...
            ?>
                ...
                <li class="sod_li">
                    ...
                    <div class="li_op_wr">
                        ...
                        <div class="total_img"><a href="<?php echo subscription_item_url($row['it_id']); ?>"><?php echo $image; ?></a></div>
            ...
            if ($i == 0) {
                ...
                // 배송비 계산
                $send_cost = get_subscription_sendcost($s_cart_id, 0);
            }
            ?>
        </ul>
        ...
        <?php if ($i == 0) { ?>
            <div class="go_shopping"><a href="<?php echo G5_SUBSCRIPTION_URL; ?>/" class="btn01">쇼핑 계속하기</a></div>
        <?php } else { ?>
            <div class="sod_ta_wr">
                <?php
                ...
                <div id="sod_bsk_act" class="btn_confirm">
                    ...
                    <input type="hidden" name="url" value="<?php echo G5_SUBSCRIPTION_URL; ?>/orderform.php">
