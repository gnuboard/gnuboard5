...
namespace API\Service\Memo;

include_once __DIR__ . '/../Service/Member.php';
 ...
 * @return int
 */
function fetchTotalCount($me_type, $mb_id)
{
 ...
 * @return array
 */
function fetchMemos(string $me_type, string $mb_id, int $page, int $per_page)
{
    if ($me_type === 'recv') {
        $where = "me_recv_mb_id = :mb_id AND me_type = :me_type AND me_read_datetime = :me_read_datetime";
    } else {
 ...
 */
function getReciveMembers(string $receiver_ids)
{
 ...
 * @throws \Exception
 */
function sendMemo($mb_id, $receiver_ids, $content, $ip)
{
    $result = getReciveMembers($receiver_ids);
    $member_result = fetchMemberById($mb_id);
 ...
 * @return array
 */
function fetchMemo($memo_id, $member_id)
{
    $memo_table = $GLOBALS['g5']['memo_table'];
 ...
 */
function deleteMemo(int $memo_id, string $mb_id)
{
    $memo_table = $GLOBALS['g5']['memo_table'];
    $query = "SELECT * FROM $memo_table WHERE me_id = :me_id";
    $memo = Db::getInstance()->run($query, ['me_id' => $memo_id])->fetch();
 ...
 */
function deleteMemoCall(int $memo_id)
{
    ...
    updateNotReadMemoCount($memo['me_recv_mb_id']);
}
 ...
 */
function updateNotReadMemoCount(string $receiver_mb_id): int
{
    $not_read_memo_count = notReadMemoCount($receiver_mb_id);
    $member_table = $GLOBALS['g5']['member_table'];
 ...
 * @return int
 */
function notReadMemoCount($mb_id)
{
    $memo_table = $GLOBALS['g5']['memo_table'];
