...
class MenuService
{
     ...
     * @return array
     */
    private function fetch_menu($is_mobile = false)
    {
     ...
     * @return array
     */
    public function sort_menu(array $data)
    {
        $parents = array_filter($data, fn($item) => strlen($item['me_code']) === 2);
        $sub_menus = array_filter($data, fn($item) => strlen($item['me_code']) === 4);

        usort($parents, fn($a, $b) => $a['me_order'] <=> $b['me_order']);
        usort($sub_menus, fn($a, $b) => $a['me_order'] <=> $b['me_order']);
     ...
     * @todo cache 적용
     */
    public function get_menus($is_mobile = false): array
    {
        $menu_data = $this->fetch_menu($is_mobile);
        if (!$menu_data) {
            return [];
        }
        return $this->sort_menu($menu_data);
    }
