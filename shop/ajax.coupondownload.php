...
// 포인트는 회원 단위 자원이므로 쿠폰이 달라도 같은 회원의 요청은 직렬화한다.
$lock_key = 'g5_coupon_dl_'.addslashes($member['mb_id']);
...
// 포인트 쿠폰은 회원포인트 체크 (락 획득 후 최신 잔액으로 재확인)
if($cp['cz_type']) {
    $row_pt = sql_fetch(" select mb_point from {$g5['member_table']} where mb_id = '{$member['mb_id']}' ");

    if(($row_pt['mb_point'] - $cp['cz_point']) < 0) {
        sql_query(" do release_lock('$lock_key') ");
