...
$cart_items = sql_result_array($results);

// 판매가격
$sql2 = " select ct_price, it_name, ct_send_cost from {$g5['g5_subscription_cart_table']} where od_id = '$od_id' and it_id = '$it_id' order by ct_id asc limit 1 ";
$row2 = sql_fetch($sql2);
...
<h2>상품옵션수정</h2>
<!-- 장바구니 옵션 시작 { -->
<form name="foption" method="post" action="<?php echo G5_SUBSCRIPTION_URL; ?>/ajax_order_cartoption_update.php" onsubmit="return formcheck(this);">
    <input type="hidden" name="act" value="optionmod">
    <input type="hidden" name="return_query" value="">
    <input type="hidden" name="od_id" value="<?php echo sanitize_input($od['od_id']); ?>">
    <input type="hidden" name="it_id[]" value="<?php echo sanitize_input($it['it_id']); ?>">
    <input type="hidden" id="it_price" value="<?php echo sanitize_input($row2['ct_price']); ?>">
    <input type="hidden" name="ct_send_cost" value="<?php echo sanitize_input($row2['ct_send_cost']); ?>">
    <input type="hidden" name="fnonce" value="<?php echo generate_form_nonce('subscription_ajax_order_cartoption'); ?>">
    <input type="hidden" name="form_id" value="subscription_ajax_order_cartoption">
    <input type="hidden" name="ct_dels" value="">
        ...
        <ul id="sit_opt_added">
            ...
            ?>
                <li class="sit_<?php echo $cls; ?>_list">
                    <input type="hidden" name="ct_id[<?php echo $it['it_id']; ?>][]" value="<?php echo sanitize_input($row['ct_id']); ?>">
                    <input type="hidden" name="io_type[<?php echo $it['it_id']; ?>][]" value="<?php echo sanitize_input($row['io_type']); ?>">
                    <input type="hidden" name="io_id[<?php echo $it['it_id']; ?>][]" value="<?php echo sanitize_input($row['io_id']); ?>">
                    <input type="hidden" name="io_value[<?php echo $it['it_id']; ?>][]" value="<?php echo sanitize_input($row['ct_option']); ?>">
                    ...
                    <div class="opt_count">
                        ...
                        <button type="button" class="sit_opt_del" data-ctid="<?php echo sanitize_input($row['ct_id']); ?>"><i class="fa fa-times" aria-hidden="true"></i><span class="sound_only">삭제</span></button>
                    </div>
    ...
    <div class="btn_confirm">
        <button type="submit" class="btn_submit">옵션 수정</button>
    </div>
</form>
