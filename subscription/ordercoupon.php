    ...
    <div id="od_coupon_frm" class="od_coupon">
        ...
        <div class="tbl_head02 tbl_wrap">
            ...
            for($i=0; $row=sql_fetch_array($result); $i++) {
                // 사용한 쿠폰인지 체크
                if(is_used_subscription_coupon($member['mb_id'], $row['cp_id']))
                    continue;
            ...
            <tr>
                <td>
                    <input type="hidden" name="o_cp_id[]" value="<?php echo sanitize_input($row['cp_id']); ?>">
                    <input type="hidden" name="o_cp_prc[]" value="<?php echo sanitize_input($dc); ?>">
                    <input type="hidden" name="o_cp_subj[]" value="<?php echo sanitize_input($row['cp_subject']); ?>">
