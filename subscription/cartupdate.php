...
// 정기구독은 회원만 구독이 가능합니다.
if (!$is_member) {
    goto_url(G5_BBS_URL . '/login.php?url=' . urlencode(G5_SUBSCRIPTION_URL . '/cart.php'));
}

// print_r2($_POST); exit;

// 보관기간이 지난 상품 삭제
subscription_cart_item_clean();
...
$post_ct_chk = (isset($_POST['ct_chk']) && is_array($_POST['ct_chk'])) ? $_POST['ct_chk'] : [];
$post_it_ids = (isset($_POST['it_id']) && is_array($_POST['it_id'])) ? $_POST['it_id'] : [];
...
if ($act == 'buy') {
        ...
        if ($ct_chk) {
            ...
            for ($k = 0; $row = sql_fetch_array($result); ++$k) {
                ...
                if (!$row['io_id']) {
                    ...
                    $it_stock_qty = get_subscription_option_stock_qty($it_id, $row['io_id'], $row['io_type']);
                }
    ...
    if ($is_member) { // 회원인 경우
        goto_url(G5_SUBSCRIPTION_URL . '/orderform.php');
    } else {
        goto_url(G5_BBS_URL . '/login.php?url=' . urlencode(G5_SUBSCRIPTION_URL . '/orderform.php'));
    }
} elseif ($act == 'alldelete') { // 모두 삭제이면
    $sql = " delete from {$g5['g5_subscription_cart_table']}
              where od_id = '$tmp_cart_id' ";
    sql_query($sql);
} elseif ($act == 'seldelete') { // 선택삭제
    if (!count($post_ct_chk)) {
    ...
    for ($i = 0; $i < $fldcnt; ++$i) {
        ...
        if ($ct_chk) {
            $it_id = isset($post_it_ids[$i]) ? safe_replace_regex($post_it_ids[$i], 'it_id') : '';
            if ($it_id) {
                $sql = " delete from {$g5['g5_subscription_cart_table']} where it_id = '$it_id' and od_id = '$tmp_cart_id' ";
                sql_query($sql);
            }
        }
    }
    ...
    $ct_count = 0;
    $post_chk_it_id = (isset($_POST['chk_it_id']) && is_array($_POST['chk_it_id'])) ? $_POST['chk_it_id'] : [];
    $post_io_ids = (isset($_POST['io_id']) && is_array($_POST['io_id'])) ? $_POST['io_id'] : [];
    $post_io_types = (isset($_POST['io_type']) && is_array($_POST['io_type'])) ? $_POST['io_type'] : [];
    $post_ct_qtys = (isset($_POST['ct_qty']) && is_array($_POST['ct_qty'])) ? $_POST['ct_qty'] : [];
    ...
    for ($i = 0; $i < $count; ++$i) {
        ...
        for ($k = 0; $k < $opt_count; ++$k) {
            ...
            if (isset($row2['ct_id']) && $row2['ct_id']) {
                ...
                if (!$io_id) {
                    $tmp_it_stock_qty = get_subscription_it_stock_qty($it_id);
                } else {
                    $tmp_it_stock_qty = get_subscription_option_stock_qty($it_id, $io_id, $row2['io_type']);
                }
                ...
                $sql3 = " update {$g5['g5_subscription_cart_table']}
