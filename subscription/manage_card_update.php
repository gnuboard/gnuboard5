...
foreach($delete_ci_ids as $ci_id) {
    ...
    $row = sql_bind_select_fetch($g5['g5_subscription_mb_cardinfo_table'], '*', array('ci_id'=>$ci_id, 'mb_id'=>$member['mb_id']));
    ...
    if (isset($row['ci_id']) && $row['ci_id']) {
        ...
        // sql_bind_delete($g5['g5_subscription_mb_cardinfo_table'], array('mb_id'=>$member['mb_id'], 'pg_service'=>$pg_service, 'od_test'=>$od_test, 'card_mask_number'=>$card_mask_number));
    }
}
