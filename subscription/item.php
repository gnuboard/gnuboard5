...
$it = get_subscription_item_with_category($it_id, $it_seo_title);
...
// 본인인증, 성인인증체크
if (!$is_admin) {
    $msg = subscription_member_cert_check($it_id, 'item');
    if ($msg)
...
if($it['it_skin']) {
    if(preg_match('#^theme/(.+)$#', $it['it_skin'], $match))
        ...
        $skin_dir = G5_PATH.'/'.G5_SKIN_DIR.'/'.G5_SUBSCRIPTION_DIR.'/'.$it['it_skin'];
...
$ori_shop_skin_path = G5_SHOP_SKIN_PATH;
$skin_dir = $ori_shop_skin_path;
$ca_dir_check = true;

$shop_skin_dir = G5_SHOP_DIR;

// 분류에서 정기결제 스킨을 사용하는지 체크한다.
if (isset($ca['ca_class_num']) && $ca['ca_class_num']) {
    // 정기결제 스킨을 사용한다면
    $ori_shop_skin_path = str_replace(G5_SKIN_DIR . '/' . G5_SHOP_DIR . '/', G5_SKIN_DIR . '/' . G5_SUBSCRIPTION_DIR . '/', $skin_dir);
    $skin_dir = $ori_shop_skin_path;
    $shop_skin_dir = G5_SUBSCRIPTION_DIR;
}

if ($it['it_skin']) {
    if (preg_match('#^theme/(.+)$#', $it['it_skin'], $match))
        $skin_dir = G5_THEME_PATH . '/' . G5_SKIN_DIR . '/' . $shop_skin_dir . '/' . $match[1];
    else
        $skin_dir = G5_PATH . '/' . G5_SKIN_DIR . '/' . $shop_skin_dir . '/' . $it['it_skin'];
...
if ($ca_dir_check) {
    if ($ca['ca_skin_dir']) {
        if (preg_match('#^theme/(.+)$#', $ca['ca_skin_dir'], $match))
            ...
            $skin_dir = G5_PATH . '/' . G5_SKIN_DIR . '/' . $shop_skin_dir . '/' . $ca['ca_skin_dir'];
...
if (!is_file($nav_skin))
    $nav_skin = $ori_shop_skin_path . '/navigation.skin.php';
include $nav_skin;
...
if (defined('G5_THEME_USE_ITEM_CATEGORY') && G5_THEME_USE_ITEM_CATEGORY) {
    ...
    if (!is_file($cate_skin))
        $cate_skin = $ori_shop_skin_path . '/listcategory.skin.php';
    include $cate_skin;
}

if ($is_admin) {
    echo '<div class="sit_admin"><a href="' . G5_ADMIN_URL . '/subscription_admin/itemform.php?w=u&amp;it_id=' . $it_id . '" class="btn_admin btn" title="상품 관리"><span class="sound_only">상품 관리</span><i class="fa fa-cog fa-spin fa-fw"></i></a></div>';
}
?>
...
<?php
// 상단 HTML
echo run_replace('subscription_it_head_html', '<div id="sit_hhtml">' . conv_content($it['it_head_html'], 1) . '</div>', $it);
...
$sns_url  = subscription_item_url($it['it_id']);
$sns_share_links = get_sns_share_link('facebook', $sns_url, $sns_title, G5_SUBSCRIPTION_SKIN_URL . '/img/facebook.png') . ' ';
$sns_share_links .= get_sns_share_link('twitter', $sns_url, $sns_title, G5_SUBSCRIPTION_SKIN_URL . '/img/twitter.png') . ' ';

// 상품품절체크
if (G5_SOLDOUT_CHECK)
    $is_soldout = subscription_is_soldout($it['it_id']);
...
<div id="sit">
    ...
    if (!is_file($info_skin))
        $info_skin = $ori_shop_skin_path . '/item.info.skin.php';
    include $info_skin;
    ?>
...
<?php
// 하단 HTML
echo run_replace('subscription_it_tail_html', conv_content($it['it_tail_html'], 1), $it);
?>
