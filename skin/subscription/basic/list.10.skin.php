...
foreach((array) $list as $row){
    ...
    $item_link_href = subscription_item_url($row['it_id']);     // 상품링크
    ...
    $is_soldout = subscription_is_soldout($row['it_id'], true);   // 품절인지 체크
    ...
    if ($this->view_it_img) {
        echo get_subscription_it_image($row['it_id'], $this->img_width, $this->img_height, '', '', stripslashes($row['it_name']))."\n";
    }
	...
	if ($this->view_it_icon) {
        // 품절
        if ($is_soldout) {
            echo '<span class="subscription_icon_soldout"><span class="soldout_txt">SOLD OUT</span></span>';
        }
    }
    echo "</div>\n";
