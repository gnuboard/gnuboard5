...
<? if ($member[mb_sex]) { ?><input type="hidden" id="mb_sex" name="mb_sex" value='<?=$member[mb_sex]?>'><? } ?>
...
<tr>
    ...
    <td>
        <input type="text" id="reg_mb_id" name="mb_id" value="<?=$member[mb_id]?>" maxlength="20" size="20" <? if ($w=='u') { echo "readonly style='background-color:#dddddd;'"; } ?> <? if ($w=='') { echo "onblur='reg_mb_id_check();'"; } ?>>
...
<tr>
    ...
    <td><input type="password" id="mb_password" name="mb_password" size=20 maxlength=20 <?=($w=="")?"required":"";?>></td>
</tr>
<tr>
    <th scope="row"><label for="">패스워드 확인</label></th>
    <td><input type="password" id="mb_password_re" name="mb_password_re" size=20 maxlength=20 <?=($w=="")?"required":"";?>></td>
</tr>
</table>
...
<tr>
    ...
    <td>
        <input id="mb_name" name="mb_name" value="<?=$member[mb_name]?>" <?=$member[mb_name]?"readonly2":"";?>>
        <? if ($w=='') { echo "(공백없이 한글만 입력 가능)"; } ?>
    </td>
</tr>
<? if ($member[mb_nick_date] <= date("Y-m-d", $g4[server_time] - ($config[cf_nick_modify] * 86400))) { // 별명수정일이 지났다면 수정가능 ?>
<input type="hidden" id="mb_nick_default" name="mb_nick_default" value='<?=$member[mb_nick]?>'>
<tr>
    <th scope="row"><label for="">별명</label></th>
    <td>
...
<? } else { ?>
<input type="hidden" id="mb_nick_default" name="mb_nick_default" value='<?=$member[mb_nick]?>'>
<input type="hidden" id="mb_nick" name="mb_nick" value="<?=$member[mb_nick]?>">
<? } ?>

<tr>
    <th scope="row"><label for="">E-mail</label></th>
    <td>
        <input type="hidden" id="old_email" name="old_email" value='<?=$member[mb_email]?>'>
        <input type="text" id='reg_mb_email' id="mb_email" name="mb_email" size=38 maxlength=100 value='<?=$member[mb_email]?>' onblur="reg_mb_email_check()">
...
<tr>
    ...
    <td><input type="text" id=mb_birth id="mb_birth" name="mb_birth" size=8 maxlength=8 minlength=8 required value='<?=$member[mb_birth]?>' readonly title='옆의 달력 아이콘을 클릭하여 날짜를 입력하세요.'></td>
</tr>
<? } ?>
...
<tr>
    ...
    <td>
        <select id="mb_sex" name="mb_sex" required>
...
<tr>
    ...
    <td><input type="text" id="mb_homepage" name="mb_homepage" size=38 maxlength=255 <?=$config[cf_req_homepage]?'required':'';?> value='<?=$member[mb_homepage]?>'></td>
</tr>
<? } ?>
...
<tr>
    ...
    <td><input type="text" id="mb_tel" name="mb_tel" size=21 maxlength=20 <?=$config[cf_req_tel]?'required':'';?> value='<?=$member[mb_tel]?>'></td>
</tr>
<? } ?>

<? if ($config[cf_use_hp]) { ?>
<tr>
    <th scope="row"><label for="">핸드폰번호</label></th>
    <td><input type="text" id="mb_hp" name="mb_hp" size=21 maxlength=20 <?=$config[cf_req_hp]?'required':'';?> value='<?=$member[mb_hp]?>'></td>
</tr>
<? } ?>
...
<tr>
    ...
    <td>
        <input type="text" id="mb_zip1" name="mb_zip1" size=4 maxlength=3 readonly <?=$config[cf_req_addr]?'required':'';?> value='<?=$member[mb_zip1]?>'>
         -
        <input type="text" id="mb_zip2" name="mb_zip2" size=4 maxlength=3 readonly <?=$config[cf_req_addr]?'required':'';?> value='<?=$member[mb_zip2]?>'>
        ...
        <input type="text" id="mb_addr1" name="mb_addr1" size=60 readonly <?=$config[cf_req_addr]?'required':'';?> value='<?=$member[mb_addr1]?>'>
        <input type="text" id="mb_addr2" name="mb_addr2" size=60 <?=$config[cf_req_addr]?'required':'';?> value='<?=$member[mb_addr2]?>'>
    </td>
</tr>
<? } ?>
</table>
...
<tr>
    <th scope="row"><label for="">서명</label></th>
    <td><textarea id="mb_signature" name="mb_signature" class=tx rows=3 style='width:95%;' <?=$config[cf_req_signature]?'required':'';?>><?=$member[mb_signature]?></textarea></td>
</tr>
<? } ?>

<? if ($config[cf_use_profile]) { ?>
<tr>
    <th scope="row"><label for="">자기소개</label></th>
    <td><textarea id="mb_profile" name="mb_profile" class=tx rows=3 style='width:95%;' <?=$config[cf_req_profile]?'required':'';?>><?=$member[mb_profile]?></textarea></td>
</tr>
<? } ?>

<? if ($member[mb_level] >= $config[cf_icon_level]) { ?>
<tr>
    <th scope="row"><label for="">회원아이콘</label></th>
    <td>
...
<tr>
    <th scope="row"><label for="">메일링서비스</label></th>
    <td><input type="checkbox" id="mb_mailling" name="mb_mailling" value='1' <?=($w=='' || $member[mb_mailling])?'checked':'';?>>정보 메일을 받겠습니다.</td>
</tr>
<tr>
    <th scope="row"><label for="">SMS 수신여부</label></th>
    <td><input type="checkbox" id="mb_sms" name="mb_sms" value='1' <?=($w=='' || $member[mb_sms])?'checked':'';?>>핸드폰 문자메세지를 받겠습니다.</td>
</tr>

<? if ($member[mb_open_date] <= date("Y-m-d", $g4[server_time] - ($config[cf_open_modify] * 86400))) { // 정보공개 수정일이 지났다면 수정가능 ?>
<input type="hidden" id="mb_open_default" name="mb_open_default" value='<?=$member[mb_open]?>'>
<tr>
    <th scope="row"><label for="">정보공개</label></th>
    <td><input type="checkbox" id="mb_open" name="mb_open" value='1' <?=($w=='' || $member[mb_open])?'checked':'';?>>다른분들이 나의 정보를 볼 수 있도록 합니다.
        <br>&nbsp;&nbsp;&nbsp;&nbsp; 정보공개를 바꾸시면 앞으로 <?=(int)$config[cf_open_modify]?>일 이내에는 변경이 안됩니다.</td>
</tr>
<? } else { ?>
<input type="hidden" id="mb_open" name="mb_open" value="<?=$member[mb_open]?>">
<tr>
    <th scope="row"><label for="">정보공개</label></th>
    <td>
...
<tr>
    <th scope="row"><label for="">추천인아이디</label></th>
    <td><input type="text" id="mb_recommend" name="mb_recommend"></td>
</tr>
<? } ?>
</table>
