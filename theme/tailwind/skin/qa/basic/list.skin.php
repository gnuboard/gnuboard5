...
<div id="bo_list">
    ...
    <div id="bo_btn_top">
        ...
        <ul class="btn_bo_user">
			    ...
			    <div class="bo_sch_wrap hidden w-full h-full fixed top-0 left-0 z-999">
				    <fieldset class="bo_sch absolute top-1/2 left-1/2 bg-white text-left w-80 max-h-80 overflow-y-auto rounded-md shadow-lg shadow-zinc-500/50 transform -translate-x-1/2 -translate-y-1/2 dark:bg-zinc-900 dark:shadow-zinc-950">
				        ...
				        <div class="sch_bar inline-block w-full mt-4 border border-gray-300 rounded shadow-inner dark:bg-zinc-800 dark:border-mainborder dark:text-white">
				       		<input type="text" name="stx" value="<?php echo stripslashes($stx); ?>" id="stx" class="sch_input w-60 h-10 border-0 p-0 bg-transparent float-left" required class="sch_input" size="25" maxlength="15" placeholder=" 검색어를 입력해주세요">
                  <button type="submit" value="검색" class="sch_btn w-10 h-10 float-right text-gray-600 bg-none border-0 text-base dark:text-gray-300" title="검색"><i class="fa fa-search" aria-hidden="true"></i><span class="sound_only">검색</span></button>
				        </div>
				        <button type="button" class="bo_sch_cls absolute right-0 top-0 text-gray-400 border-0 text-base bg-white py-3 px-4 dark:bg-zinc-900"><i class="fa fa-times" aria-hidden="true"></i><span class="sound_only">닫기</span></button>
				        </form>
				    </fieldset>
            ...
            <?php if ($write_href) { ?><li class="dark:bg-zinc-900"><a href="<?php echo $write_href ?>" class="btn_b01 btn dark:hover:text-white" title="문의등록"><i class="fa fa-pencil" aria-hidden="true"></i><span class="sound_only">문의등록</span></a></li><?php } ?>
        </ul>
        <?php } ?>
    </div>
    <!-- } 게시판 페이지 정보 및 버튼 끝 -->
    ...
    <div class="bo_fx">
        <ul class="btn_bo_user">
        	<?php if ($is_checkbox) { ?>
            <li class="dark:bg-zinc-900"><button type="submit" name="btn_submit" value="선택삭제" title="선택삭제" onclick="document.pressed=this.value" class="btn btn_b01 btn_admin"><i class="fa fa-trash-o" aria-hidden="true"></i><span class="sound_only">선택삭제</span></button></li>
            <?php } ?>
            <?php if ($list_href) { ?><li class="dark:bg-zinc-900"><a href="<?php echo $list_href ?>" class="btn_b01 btn dark:hover:text-white" title="목록"><i class="fa fa-list" aria-hidden="true"></i><span class="sound_only">목록</span></a></li><?php } ?>
            <?php if ($write_href) { ?><li class="dark:bg-zinc-900"><a href="<?php echo $write_href ?>" class="btn_b01 btn dark:hover:text-white" title="문의등록"><i class="fa fa-pencil" aria-hidden="true"></i><span class="sound_only">문의등록</span></a></li><?php } ?>
        </ul>
    </div>
    </form>
