...
@section('title', '글수정 - ' . $board->bo_subject)
...
<div class="container">
    ...
    <div class="card">
        <div class="card-header">
            <h4 class="mb-0">글수정</h4>
        </div>
        <div class="card-body">
            <form method="POST" action="{{ route('board.update', [$board->bo_table, $post->wr_id]) }}" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                ...
                <div class="mb-3">
                    ...
                    <select class="form-select" id="ca_name" name="ca_name">
                        ...
                        @foreach($board->categories as $category)
                            <option value="{{ $category }}" {{ old('ca_name', $post->ca_name) == $category ? 'selected' : '' }}>
                                {{ $category }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control @error('wr_subject') is-invalid @enderror" 
                           id="wr_subject" name="wr_subject" value="{{ old('wr_subject', $post->wr_subject) }}" required>
                    @error('wr_subject')
                ...
                <div class="mb-3">
                    ...
                    <textarea class="form-control @error('wr_content') is-invalid @enderror" 
                              id="wr_content" name="wr_content" rows="10" required>{{ old('wr_content', $post->wr_content) }}</textarea>
                    @error('wr_content')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="mb-3">
                    ...
                    <input type="url" class="form-control @error('wr_link1') is-invalid @enderror" 
                           id="wr_link1" name="wr_link1" value="{{ old('wr_link1', $post->wr_link1) }}" placeholder="http://">
                    @error('wr_link1')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="mb-3">
                    ...
                    <input type="url" class="form-control @error('wr_link2') is-invalid @enderror" 
                           id="wr_link2" name="wr_link2" value="{{ old('wr_link2', $post->wr_link2) }}" placeholder="http://">
                    @error('wr_link2')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                    ...
                    <div>
                        ...
                        <div class="form-check form-check-inline">
                            <input class="form-check-input" type="checkbox" id="secret" name="secret" value="1" 
                                   {{ $post->isSecret() ? 'checked' : '' }}>
                            <label class="form-check-label" for="secret">비밀글</label>
                        </div>
                        @endif
                        @if($board->bo_use_dhtml_editor)
                        <div class="form-check form-check-inline">
                            <input class="form-check-input" type="checkbox" id="html" name="html" value="1" 
                                   {{ $post->isHtml() ? 'checked' : '' }}>
                        ...
                        <div class="form-check form-check-inline">
                            <input class="form-check-input" type="checkbox" id="mail" name="mail" value="1"
                                   {{ strpos($post->wr_option, 'mail') !== false ? 'checked' : '' }}>
                ...
                <div class="d-flex justify-content-between">
                    <a href="{{ route('board.show', [$board->bo_table, $post->wr_id]) }}" class="btn btn-secondary">취소</a>
